...
The current release is `0.1.0-alpha.1`. Aggressive transformations are opt-in. Cache correctness and prevention of private commerce-page caching take priority over cache hit rate.
...
- Reversible `WP_CACHE` management and drop-in ownership checks.
- Cloudflare Free setup through one managed Cache Rule, origin-aware TTLs, URL/full purge, encrypted token storage, rule backup, and automatic fallback when a Free zone rejects custom query-string cache keys.
...
- Optional: Cloudflare proxied DNS and an API token
...
Create a scoped token for the site’s zone with:

- Zone read access if GT Performance should discover the zone ID;
- Cache Rules edit access;
- Cache purge access.

Enter the token under **Settings → GT Performance**, then select **Connect/sync Cloudflare**. GT Performance uses the normal Cloudflare CDN fetch path and Cache Rules; it does not require APO, Workers, Cache Reserve, Argo, or an Enterprise plan.
...
This is an alpha for disposable-site and controlled staging validation. Cloudflare mutations require a real scoped token and are not exercised by the offline test suite. FluentCart, EDD, WooCommerce, multisite, and host-cache combinations still need a growing compatibility matrix before a stable release.
