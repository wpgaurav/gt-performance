...
* This build migrates automatically. Updating from 1.0.0 or earlier replaces the old cache drop-in, removes the configuration files and database tables earlier releases left behind, and reports any `GTP_` constants in `wp-config.php` that need renaming to `GTPERF_`.
* If a site was already updated past 1.0.0 by another route and now shows a blank page, run the standalone repair script first: https://gist.github.com/wpgaurav/03d61d313df00b4127db92393ed74681
