<?php
/**
 * Purpose-bound encryption for saved plugin secrets.
 *
...
final class SecretCipher {
	...
	public function encrypt( string $plain ): string {
		...
		if ( false === $encrypted ) {
			throw new \RuntimeException( 'Unable to encrypt the saved secret.' );
		}

		return 'openssl:' . base64_encode( $nonce . $tag . $encrypted );
	}

	public function decrypt( string $stored, string $constantName = '' ): string {
	...
	private function key(): string {
		$material = ( defined( 'AUTH_KEY' ) ? AUTH_KEY : '' ) . '|' .
			( defined( 'SECURE_AUTH_SALT' ) ? SECURE_AUTH_SALT : '' ) . '|gt-performance-' . $this->purpose;
