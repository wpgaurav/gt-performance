...
final class LicenseManager {
	...
	public function activate( string $licenseKey ): bool|\WP_Error {
		...
		if ( '' === $licenseKey ) {
			return new \WP_Error( 'gtperf_license_key', __( 'Enter a GT Performance license key.', 'gt-performance' ) );
		}
		...
		if ( ! in_array( $status, array( 'valid', 'active' ), true ) ) {
			return new \WP_Error( 'gtperf_license_invalid', __( 'FluentCart did not accept this license for the current site.', 'gt-performance' ) );
		}

		try {
			$this->repository->saveActivation( $licenseKey, $response );
		} catch ( \Throwable ) {
			return new \WP_Error( 'gtperf_license_save', __( 'GT Performance could not encrypt and save the license.', 'gt-performance' ) );
		}
	...
	public function verify(): bool|\WP_Error {
		...
		$status = sanitize_key( (string) ( $response['status'] ?? '' ) );
		if ( ! in_array( $status, array( 'valid', 'active' ), true ) ) {
			return new \WP_Error( 'gtperf_license_invalid', __( 'The saved GT Performance license is not valid for this site.', 'gt-performance' ) );
		}
	...
	public function deactivate(): bool|\WP_Error {
		...
		if ( '' === $key ) {
			return new \WP_Error( 'gtperf_license_key', __( 'No saved license key is available to deactivate.', 'gt-performance' ) );
		}
		...
		if ( ! in_array( $status, array( 'deactivated', 'inactive' ), true ) ) {
			return new \WP_Error( 'gtperf_license_deactivate', __( 'FluentCart did not confirm license deactivation.', 'gt-performance' ) );
		}
	...
	public function credentials(): array|\WP_Error {
		...
		if ( '' === $key && '' === $hash ) {
			return new \WP_Error( 'gtperf_license_missing', __( 'Activate a GT Performance license before checking for updates.', 'gt-performance' ) );
		}
