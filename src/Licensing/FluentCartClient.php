...
final class FluentCartClient {
	...
	public function request( string $action, array $payload = array() ): array|\WP_Error {
		if ( $this->configuration->itemId() < 1 ) {
			return new \WP_Error( 'gtperf_license_product', __( 'The GT Performance product is not configured.', 'gt-performance' ) );
		}
		...
		$body = array_merge(
			array(
				...
				'current_version'  => GTPERF_VERSION,
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error( 'gtperf_license_connection', __( 'GT Performance could not reach the license server.', 'gt-performance' ) );
		}

		$status = (int) wp_remote_retrieve_response_code( $response );
		$data   = json_decode( (string) wp_remote_retrieve_body( $response ), true );
		if ( ! is_array( $data ) ) {
			return new \WP_Error( 'gtperf_license_response', __( 'The license server returned an unreadable response.', 'gt-performance' ) );
		}
