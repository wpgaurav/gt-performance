...
final class Updater {
	...
	public function injectUpdate( mixed $transient ): mixed {
		...
		try {
			...
			if (
				...
				'' !== $metadata['package'] &&
				version_compare( $metadata['new_version'], GTPERF_VERSION, '>' )
			) {
				$transient->response[ GTPERF_BASENAME ] = $entry;
				unset( $transient->no_update[ GTPERF_BASENAME ] );
			} else {
				$transient->no_update[ GTPERF_BASENAME ] = $entry;
				unset( $transient->response[ GTPERF_BASENAME ] );
			}
	...
	public function metadata( bool $force = false ): array|\WP_Error|null {
		...
		try {
			...
			if ( null === $metadata ) {
				return new \WP_Error( 'gtperf_license_update_response', __( 'The license server returned incomplete update details.', 'gt-performance' ) );
			}
	...
	private static function cacheKey(): string {
		...
		return 'gtperf_fluentcart_update_' . md5( GTPERF_BASENAME . '|' . (string) $itemId );
	}
}
